function solution() {
    ...
    function addGift() {
        ...
        const send = e('button', 'Send', 'sendButton')
        const discard = e('button', 'Discard', 'discardButton')
        element.appendChild(send)
        element.appendChild(discard)

        send.addEventListener('click', () => sendGift(name, element));
        discard.addEventListener('click', () => discardGift(name, element));
